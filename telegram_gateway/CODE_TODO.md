...
Status: **Resolved (2026-09-16).** Raised directly by the user testing the live bot ("bot did not provide any response when clicking on a button", ~07:13am) - traced against `telegram_gateway.log`/`bot_sanctuary.log` for that morning (both logs read in full, cross-referenced by timestamp/`task_id`) before any fix was scoped, per explicit follow-up instruction to scope all three issues found that morning and log them here. Scoped, then implemented, both same day - see "Implementation Notes (2026-09-16)" below for what actually shipped, including one corrected claim from the original scoping.
...
- **New finding, found while tracing this fix through to its actual consumer - changes this entry's scope materially: `bot_sanctuary`'s own turn-building step doesn't read anything but `text` from a task payload today.** `utils_session/session_worker.py::_process_batch()` builds its combined prompt with `combined_text = "\n".join(text for text in ((item.get("text") or "") for item in batch) if text.strip())` - `image_url`/`video_url`/`file_url`/`poll_answer` are *all* already just as inert as a hypothetical `button_press` field would be; none of them are read anywhere in `_process_batch()` or downstream in `call_dispatch_handler.py`. ~~This means `telegram_gateway`'s side of the poll-answer round trip (`_push_poll_answer()`) has apparently never had a working consumer on `bot_sanctuary`'s side either - the same gap this entry originally found for buttons turns out to already exist for polls, undocumented until now.~~ **Corrected 2026-09-16, same day - this specific claim about poll_answer was wrong.** Challenged directly by the user ("how is poll_answer responding? I recall testing it successfully") - re-investigation across every poll test in every log that day (not just the first, which is all the original claim above was based on) found **3 genuinely successful poll_answer round-trips** that same morning; the one failing poll test that prompted the original claim failed because of an unrelated app restart landing between it and the successful ones, not a missing consumer. `_process_batch()`'s "only reads `text`" gap is real and was still worth fixing generally (see below), but it was never true that poll_answer "never had a working consumer" - that specific claim is retracted, not just superseded. **This fix is therefore a two-repo change, not a `telegram_gateway`-only one**: `_process_batch()` (or wherever the final prompt is actually assembled before reaching Claude) needs a new branch that recognises a `button_press` (and, while there, `poll_answer`) key on a batch item and folds it into the turn's prompt as something like "the user selected: <purpose>/<payload>" - otherwise the press would arrive, be correctly resolved back onto its `task_id`, and still produce no visible effect.
...
3. ~~Whether to fix `_process_batch()`'s "only reads `text`" gap generally (covering `poll_answer`/media too, since all are equally affected) as part of this same change, or narrowly.~~ **Resolved - broad.** Implemented in `bot_sanctuary`'s `_extract_item_text()` covering `button_press`, `poll_answer`, and (per the delivery_failed entry below) `delivery_failed` all in one function - see `bot_sanctuary/CODE_TODO.md`'s own entry for the implementation.
4. Whether any button `purpose` besides `select_shoe`/`draft_continue` is expected soon - affects only how the persona-facing wording of the folded-in prompt ("the user selected: ...") should be phrased generically enough to cover future purposes, not whether the mechanism itself is worth building. Still open, non-blocking.

### Implementation Notes (2026-09-16)

- `register_bot_button()` (`button_prompt_handler.py`) gained the planned `task_id: str | None = None` parameter, stored in `_registered_callbacks[token]`.
- `validate_bot_callback()` now returns `{"purpose", "payload", "task_id"}`.
- `_build_button_rows()` (`message_handler.py`) threads `task_id` through into each `register_bot_button()` call.
- New `_push_button_press(chat_id, purpose, payload, task_id)` (`gateway_inbound.py`) - mirrors `poll_response_handler.py::_push_poll_answer()`'s task_id-reuse pattern exactly, as planned. `_handle_update()`'s callback branch calls it for any `purpose` other than `"draft_continue"`.
- **Separate, related gap identified but deliberately not fixed as part of this entry: `answerCallbackQuery` is never called anywhere in this codebase.** Confirmed against Telegram's own Bot API documentation this is the required call to dismiss a pressed button's client-side loading spinner - independent of the routing gap this entry fixes. **Evaluated and explicitly declined by the user** after confirming its effect is purely cosmetic (the spinner eventually resolves client-side on its own; no functional/delivery consequence either way, no retry/redelivery is affected by leaving it uncalled) - judged not worth implementing for the value delivered. Not tracked as a pending item; would need a fresh explicit request to reopen.
- Cross-reference: `bot_sanctuary/CODE_TODO.md`'s `_process_batch()` entry (now resolved) - this fix does not work end-to-end without that half, which has since landed.

### Follow-up Work

- None outstanding for this entry. `answerCallbackQuery` is a separate, declined item (see above), not a follow-up of this fix.

### Where

- `telegram_gateway_application/utilities/utils_telegram/utilities/button_prompt_handler.py`: `register_bot_button()` (new `task_id` param), `validate_bot_callback()` (returns `task_id`).
- `telegram_gateway_application/utilities/utils_queue/message_handler.py`: `_build_button_rows()` (threads `task_id` through).
- `telegram_gateway_application/utilities/utils_telegram/gateway_inbound.py`: new `_push_button_press()`, `_handle_update()`'s callback branch.
- Cross-reference: `bot_sanctuary/CODE_TODO.md`'s `_process_batch()` entry (resolved, same day).
...
Status: **Resolved (2026-09-16).** Raised directly by the user ("bot failed to respond to a message", ~09:17am) - traced against both logs for `task_id=9478f79c8fd74cdbb6dd71aa1f0ec650`, per the same instruction as the entry above. Both (a) and (b) implemented same day - (b) shipped as a materially simpler mechanism than either candidate shape originally drafted below; see "Implementation Notes (2026-09-16)".
...
- ~~(b1) is the currently-preferred direction - it requires a change only on `bot_sanctuary`'s side (which already owns `task_id` minting for a turn) rather than new gateway-side state.~~ **Superseded 2026-09-16 - factually wrong premise, caught during design.** `bot_sanctuary` does not, and never did, mint `task_id`s - `create_task_mapping()` is exclusively called on `telegram_gateway`'s side (`gateway_inbound.py`); `bot_sanctuary` only ever reuses a `task_id` it was already handed. (b1) as drafted was architecturally impossible, not just de-prioritised. See "Implementation Notes" below for what was actually built instead - closer in spirit to (b2), but simpler than either candidate shape drafted at scoping time.

### Open Questions

1. ~~Whether (b) is worth building immediately given (a) closes today's specific trigger.~~ **Resolved - both built same day.**
2. ~~If (b1) is adopted, the exact mechanism `bot_sanctuary`'s `delivery_failed` handling would use to obtain a fresh `task_id`.~~ **Moot - (b1) was never viable, see the strike-through above. No fresh `task_id` is ever minted for a retry; the original one is kept alive instead.**

### Implementation Notes (2026-09-16)

**(a) - `_split_for_telegram()`/`_split_oversized_line()` (`message_handler.py`).** Raw text is length-checked and split *before* Markdown→HTML conversion, then each resulting chunk is converted independently - not the reverse (convert once, then slice the HTML). This was a deliberate design correction mid-implementation: slicing already-converted HTML risked cutting a `<b>`/`<i>`/`<code>` tag in half at the cut point; splitting the raw text first means a Markdown delimiter separated by a cut simply fails to find its match within its own chunk and degrades to a literal character (per `markdown_converter.py`'s own documented "degrades gracefully" guarantee) - never an unbalanced tag reaching Telegram. Lines are packed as full as possible per chunk before starting a new one; a single line still too long on its own recursively bisects. `_handle_text()`'s plain-send path now sends every returned chunk in order, reporting Tier 1 on the first rejected chunk and stopping.

**(b) - kept the *same* `task_id` alive via a `pending_retry` marker, not a fresh one.** Final design, arrived at after the (b1) correction above and one further simplification pass:
- `push_tier1_delivery_failed()` (`error_handling.py`) now calls a new `set_pending_retry(task_id)` (`database.py`) immediately after successfully pushing the `delivery_failed` event - a `pending_retry:<task_id>` Redis marker, TTL'd via `REDIS_TASK_MAPPING_TTL_SECONDS`. Not set at all if the push itself fails (no retry is realistically coming for an event that never reached `bot_sanctuary`).
- `_handle_completed()` (`message_handler.py`) now calls a new `pop_pending_retry(task_id)` (`database.py`) **before** `delete_task_mapping()`. If a retry is outstanding, the mapping is deliberately left alive (`delete_task_mapping()`/`resolve_pending_reset_if_ready()` both skipped) instead of deleted - the corrective reply `bot_sanctuary` is about to publish for this same `task_id` still needs it. That retry's own eventual `completed` finds no marker left (already consumed) and deletes for real.
- **`_handle_error()` deliberately not given the same treatment** - confirmed with the user directly: an `error` marker means `bot_sanctuary` hit a blocking failure of its own, with no corrective retry ever coming for that `task_id`, unlike a Tier 1 `delivery_failed`, which specifically exists to request one.
- **A "Scenario B" fallback (`restore_task_mapping()`, recreating an already-deleted mapping from a `chat_id` carried on the retry marker) was designed, then dropped before implementation** - validated directly against `queue.py::queue_consume_task()`'s consumer, which uses `pika`'s `BlockingChannel.start_consuming()` on a single dedicated thread with no prefetch/thread-pool concurrent dispatch. This means `process_message()` runs every message (including `push_tier1_delivery_failed()`'s own marker write) to completion before the next queued message is even dequeued - the ordering race the fallback was designed to guard against (`completed` beating the marker write) cannot occur in this codebase's actual consumption model. Dropping it removed the need for `chat_id` on the marker entirely, and for storing/reading `user_id` anywhere in this mechanism (confirmed unused by every existing caller of `get_task_mapping()`).
- Cross-reference: the retry's *content* (what Claude actually says in its corrective reply) depends on `bot_sanctuary`'s own `_extract_item_text()` recognising a `delivery_failed` payload at all - see `bot_sanctuary/CODE_TODO.md`'s `_process_batch()` entry, resolved the same day.

**Separate, narrower gap - scoped in full, under direct supervision, ahead of (b)'s own design; fix itself not yet implemented.** `_send_media_with_caption()`'s overflow `remainder` (the follow-up plain-text message sent after an over-long caption is truncated) has no length check of its own, and its `send_message()` result isn't checked/reported as a Tier 1 event at all. **Scoping question directed and confirmed by the user before (b)'s design began** ("verify it's usage for captions", "what's the possibility of the bot sending HTML into caption?"): checked every media-sending call site (`send_photo`/`send_video`/`send_document`, `_send_media_with_caption()`) and confirmed **none of them ever set `parse_mode` anywhere in this codebase**. That rules out fix (a)'s own HTML-tag-splitting risk here entirely, by construction - a caption/remainder is always sent as plain, unparsed text, so there is no tag to ever cut in half. The scoped, confirmed risk is narrower and purely length-based: an extreme `remainder` could still exceed `TELEGRAM_MESSAGE_MAX_LENGTH` and get a plain 400, uncaught and unreported as Tier 1. **Not yet fixed** - the scoping/risk-assessment is complete and conclusive; only the actual guard (a length check + Tier 1 report on the remainder, likely reusing `_split_for_telegram()`'s own chunking) remains to be decided and implemented.

### Follow-up Work

- Implement the length guard + Tier 1 reporting on `_send_media_with_caption()`'s `remainder` send, per the scoped/confirmed risk above - scoping is done, only the fix itself is outstanding.

### Where

- `telegram_gateway_application/utilities/utils_queue/message_handler.py`: `_split_for_telegram()`, `_split_oversized_line()`, `_handle_text()` (fix a); `_handle_completed()` (fix b).
