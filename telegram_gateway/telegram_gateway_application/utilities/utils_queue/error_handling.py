...
# Description : Builds and pushes delivery-failure events onto Q_CHANNEL_OUT, split
#               into Tier 1 (per-task, actionable) and Tier 2 (systemic, not
#               actionable by any single task) failures.
# Author      : SorinoSSK
# Created On  : 2026-09-02
#
# Features    :
#   - Tier 1 - push_tier1_delivery_failed(): a specific send was rejected by Telegram,
#     or failed local validation before ever reaching Telegram. Reported per task_id,
...
#
# Notes       :
#   - Deferred import of queue_push_task to avoid a circular import (queue.py ->
#     message_handler.py -> ... -> queue.py), same pattern as
#     utils_telegram/utilities/poll_response_handler.py::_push_poll_answer().
#   - Tier 2's counter/armed-state is in-memory only and resets on restart -
#     acceptable, since a restart is itself a fresh start at reassessing whether
...
def push_tier1_delivery_failed(task_id: str, attempted_type: str, status_code: int | None, reason: str) -> bool:
    ...
    Notes:
        - Used only when a specific request was rejected (by Telegram, or by local
          validation) - never for connection-level failures, which carry no "wrong
          tool" signal and are reported via record_send_failure() instead.
    """
...
def record_send_success() -> None:
    ...
    Notes:
        - Intended to be called after any successful send - clears the slate so a
          past incident doesn't suppress the next genuine one.
    """
...
def record_send_failure(reason: str, status_code: int | None = None) -> None:
    ...
    Notes:
        ...
        - Fires once per incident: re-armed only by record_send_success(), not by
          further failures, so an ongoing outage doesn't spam one alert per message.
    """
    global _consecutive_failures, _alert_armed
...
def _push_tier2_gateway_alert(reason: str, status_code: int | None) -> bool:
    ...
    Notes:
        - task_id is deliberately None - this isn't about any single task, so
          consumers of Q_CHANNEL_OUT need to expect a task_id-less message of
          type="gateway_alert".
        - Always logged at CRITICAL first, regardless of the queue push outcome, so
