...
def _config_failure_reason(status_code: int | None) -> str | None:
    ...
    Returns:
        str | None:
            "unauthorized" for 401 (invalid/revoked bot token), "not_found" for 404 (every endpoint hit here is a fixed, hardcoded path, so a 404 can't mean "wrong URL" - it means the token doesn't resolve to a real bot).
            None otherwise.

    Notes:
        - Both are permanent, config-level failures - no per-task retry or different request content fixes either, so callers treat them identically to Tier 2 (see error_handling.py::record_send_failure()), not as a Tier 1 rejection.
    """
    if status_code == 401:
        return "unauthorized"
    if status_code == 404:
...
def send_message(chat_id: int | str, text: str, parse_mode: str | None = None, reply_markup: dict | None = None) -> bool | dict:
    ...
    Returns:
        bool | dict:
            ...
            {"error": True, "status_code", "reason"} if Telegram rejected the request (Tier 1 - see module Notes).
            False if unreachable, unauthorized, or not found (Tier 2 - already recorded internally, nothing further for the caller to report).
...
def send_typing_action(chat_id: int | str) -> bool:
    ...
    Notes:
        ...
        - Not Tier 1-eligible - ephemeral, non-critical, not worth reporting per-task.
          Still feeds Tier 2 (record_send_success()/record_send_failure()) on a connection failure or 401, since either is still evidence towards a systemic issue regardless of which endpoint hit it.
    """
...
def send_poll(chat_id: int | str, question: str, options: list, allows_multiple_answers: bool = False) -> dict | None:
    ...
    Notes:
        - is_anonymous is not caller-configurable - always settings.TELEGRAM_POLL_ANONYMOUS, since correlating an answer back to a responder (see utils_telegram/utilities/poll_response_handler.py) requires a non-anonymous poll.
        - No open_period/close_date is set - Telegram caps these at 600s and they can't be adjusted once sent, which doesn't fit the debounced/extendable closing handled by poll_response_handler.py.
          The poll is closed explicitly via stop_poll() instead.
    """
...
def stop_poll(chat_id: int | str, message_id: int) -> bool:
    ...
    Notes:
        ...
        - Also returns False (logged, not retried) if the poll was already closed - callers that treat closing as best-effort cleanup should not fail hard on this.
        - Not Tier 1-eligible - closing is best-effort cleanup, not a delivery.
          Still feeds Tier 2 on a connection failure or 401 (see gateway_outbound.py module Notes).
    """
...
def send_document(chat_id: int | str, url: str, caption: str | None = None) -> bool:
    ...
    Returns:
        bool | dict:
            True if sent successfully.
            {"error": True, "status_code", "reason"} if Telegram rejected the request (Tier 1 - see module Notes).
            False if unreachable, unauthorized, or not found (Tier 2 - already recorded internally).
