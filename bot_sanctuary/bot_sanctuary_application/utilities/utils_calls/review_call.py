# =============================================================================
# File        : review_call.py
# Description : Review Call - the code/change review role in the multi-agent "Call" model.
# Author      : SorinoSSK
# Created On  : 2026-09-10
#
# Features    :
#   - handle() - sends a prompt to this Call's configured LLM (settings.LLM_REVIEW_TYPE), together with the
...
#   - Persona content lives at bot_sanctuary_application/libraries/<llm_type>/review.md, one file per
...
async def handle(prompt: str) -> str | None:
    """
    Handles a single turn for the Review Call.
    ...
    Returns:
        str | None:
            The assistant's reply, or None if LLM_REVIEW_TYPE (which falls back to LLM_CHAT_TYPE if unset)
            is unset, or the underlying call failed.

    Notes:
        - Persona content is loaded fresh from libraries/<llm_type>/review.md on every call (currently
    ...
    """
    if not settings.LLM_REVIEW_TYPE:
        logger.warning(f"{CALL_NAME} Call has no LLM_REVIEW_TYPE (or fallback LLM_CHAT_TYPE) configured - skipping.")
        return None
    else:
        persona = load_persona(settings.LLM_REVIEW_TYPE, CALL_NAME)
        return await query_llm(settings.LLM_REVIEW_TYPE, prompt, persona=persona)
