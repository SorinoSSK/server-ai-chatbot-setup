...
#
# Features    :
#   - handle() - sends a prompt to this Call's configured LLM (settings.LLM_DOCUMENTATION_TYPE), together
#     with the persona/agent-context content loaded for whichever provider that resolves to (currently
#     empty for every provider - not yet written).
#
# Notes       :
...
#   - Persona content lives at bot_sanctuary_application/libraries/<llm_type>/documentation.md, one file
#     per provider (see chat_call.py's own Notes for the full design) - loaded via
...
async def handle(prompt: str) -> str | None:
    ...
    Returns:
        str | None:
            The assistant's reply, or None if LLM_DOCUMENTATION_TYPE (which falls back to LLM_CHAT_TYPE if
            unset) is unset, or the underlying call failed.

    Notes:
        - Persona content is loaded fresh from libraries/<llm_type>/documentation.md on every call
          (currently empty for every provider) and passed as query_llm()'s persona argument - see
          chat_call.py's own Notes for how each provider delivers a non-empty one natively.
    """
