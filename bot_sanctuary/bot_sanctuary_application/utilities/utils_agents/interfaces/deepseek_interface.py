...
#     2. Age - DEEPSEEK_SESSION_TTL_DAYS (config.py, default 2) discards the whole transcript once it is that old, measured from its own original creation (absolute, not reset by activity) - a plain day-count cutoff, same as qwen_interface.py's own TTL, see _session_expiry_cutoff(). Not aligned to settings.SESSION_RESET_TIME, since that reset already wipes the transcript - alignment removed 2026-09-20. Added 2026-09-20.
...
#   - Transient failures (HTTP 429/500/502/503/504, and connection-level errors other than a timeout) are retried by api_retry.py::run_with_retry(), which wraps the whole asyncio.wait_for() call, so every attempt gets its own _OUTER_TIMEOUT_SECONDS bound. _post_chat_completion() only classifies the failure ("retryable") and never retries itself. A timeout, inner or outer, is never retried, and exhausted retries return None exactly as any other failure always has. Added 2026-09-20.
#   - An exhausted balance (HTTP 402) is never retried and is the one failure that does not return None - query_via_api() returns agent_errors.billing_exhausted_response() instead, an "error" tool reply dict (error_type="billing_exhausted") that flows through call_dispatch_handler.py::message_dissect() like an agent's own error reply. Every other failure still returns None. Added 2026-09-20.
#   - A context-length rejection (HTTP 400 whose body mentions the context length) is the one 400 that is retried, once, after dropping the oldest half of the remembered turns (whole turns only, see _drop_oldest_half()) - mirroring qwen_interface.py's own one-time fresh-conversation retry. The transcript is trimmed by bytes (DEEPSEEK_TRANSCRIPT_MAX_BYTES), not tokens, so a cap set above the model's context window would otherwise fail every turn until the TTL or a reset, since nothing is persisted after a failure. The trimmed history is persisted only if the retry succeeds. Detection is a keyword guess from the OpenAI-compatible error convention, unconfirmed against a real DeepSeek response - see _is_context_length_error() and CODE_TODO.md. Added 2026-09-20.
#   - A successful call logs DeepSeek's own usage block (token counts, including cache hit/miss) at info, with the model, so cost can be tracked from the log alone. Log-only, logged as-is. Retry log lines carry the session_dir. Both added 2026-09-20.
#   - A 200 response whose reply is empty, or whose finish_reason is anything other than "stop" (e.g. "length" or "content_filter"), is logged at warning with the reason. Log-only - the reply is still returned or None exactly as before, and it is never retried. Added 2026-09-20.
#   - The transcript read-modify-write in query_via_api() takes no lock. Each session has one SessionWorker thread (utils_session/session_worker.py) that runs its turns one at a time, and call_dispatch_handler.py runs each turn's hops sequentially, so two calls for the same session_dir do not overlap in normal operation or during a global reset sweep. One known exception: utils_queue/message_handler.py::_handle_session_cleared() removes the worker from the registry before retire() lets its in-flight turn finish, so a message arriving in that window creates a second worker for the same session_id and session_dir. The two turns can then overlap here, and the first worker's cleanup afterwards removes the second worker's registry entry and wipes the shared directory. The cause is in session_worker.py/message_handler.py, and a lock in this file would only cover the overlap, not the wipe - see CODE_TODO.md. Checked by reading the code 2026-09-20.
...
def _session_expiry_cutoff(created_at: datetime) -> datetime:
    ...
    Returns:
        datetime:
            created_at plus settings.DEEPSEEK_SESSION_TTL_DAYS.

    Notes:
        - Deliberately not aligned to settings.SESSION_RESET_TIME. The scheduled reset (utils_session/session_worker.py::trigger_timed_session_reset()) already removes the whole session_dir, this transcript included, so the TTL is only a backstop for when no reset is configured, one is skipped, or its clearing fails. Alignment was removed 2026-09-20 - see CODE_TODO.md.
        - Mirrors qwen_interface.py::_session_expiry_cutoff(). It is kept as a separate one-line copy rather than a shared helper, since each reads its own provider's TTL setting - see CODE_TODO.md.
    """
    return created_at + timedelta(days=settings.DEEPSEEK_SESSION_TTL_DAYS)
...
def _build_messages(persona: str | None, turns: list[dict], prompt: str) -> list[dict]:
    """
    Assembles the full message list to send - persona, then every remembered turn, then this turn's own prompt.

    Args:
        persona (str | None):
            The resolved persona text. Omitted from the list when empty/None.

        turns (list[dict]):
            The remembered transcript turns, oldest first, already role="user"/"assistant".

        prompt (str):
            This turn's own prompt.

    Returns:
        list[dict]:
            A fresh list - "system" persona (if any), turns as-is, then the prompt as "user".

    Notes:
        - Factored out of query_via_api() so the context-length retry can rebuild the list from the trimmed turns.
    """
    messages = []
...
def _is_context_length_error(status_code: int, body_text: str) -> bool:
    """
    Best-effort heuristic deciding whether an HTTPError means the request was larger than the model's context window.

    Args:
        status_code (int):
            The HTTP status code DeepSeek returned.
    ...
    Returns:
        bool:
            True if this looks like a context-length rejection; False for every other failure.

    Notes:
        - Unconfirmed against a real DeepSeek response - it checks for a 400 whose body mentions "context length", "context_length" or "maximum context", the wording the OpenAI-compatible convention uses. Refine once a real error body is seen.
        - Deliberately narrow (status code AND keyword), so an unrelated 400 such as a bad model name stays an ordinary permanent failure.
    """
    lowered = body_text.lower()
    return status_code == 400 and ("context length" in lowered or "context_length" in lowered or "maximum context" in lowered)

def _drop_oldest_half(turns: list[dict]) -> int:
    """
    Drops the oldest half of turns, in whole turns, in place.

    Args:
        turns (list[dict]):
            The transcript's turns, oldest first - each turn is exactly two entries (a "user" entry and its "assistant" reply).

    Returns:
        int:
            How many whole turns were dropped. 0 if there were fewer than two turns, in which case turns is left untouched.

    Notes:
        - Never drops the only remaining turn - a single oversized turn cannot be fixed by trimming history.
        - Removes entries from the list it is given, in place.
    """
    turn_count = len(turns) // 2
    if turn_count < 2:
        return 0
    else:
        dropped_turns = turn_count // 2
        del turns[0:dropped_turns * 2]
        return dropped_turns

def _post_chat_completion(messages: list[dict], token: str, model: str, reasoning_effort: str | None) -> dict:
    """
    ...
    Returns:
        dict:
            {"reply": str | None, "retryable": bool, "billing_failure": bool, "context_length_exceeded": bool}. "reply" is the assistant's reply text, or None if the request failed or returned no content. "retryable" is True only for a transient failure (see api_retry.RETRYABLE_STATUS_CODES, or a connection-level error that is not a timeout). "billing_failure" is True only when the account's balance is exhausted (see agent_errors.is_billing_failure()) - such a failure is never retryable. "context_length_exceeded" is True only when the failure looks like the request being larger than the model's context window (see _is_context_length_error()) - also never "retryable" here, since query_via_api() handles it itself, once, after trimming the history.
    ...
    try:
        ...
        logger.info(f"DeepSeek usage: model={model} usage={payload.get('usage')}")
        choice = payload["choices"][0]
        reply = choice["message"]["content"] or None
        finish_reason = choice.get("finish_reason")
        if reply is None or finish_reason != "stop":
            logger.warning(f"DeepSeek returned HTTP 200 but the reply looks incomplete or empty: model={model} finish_reason={finish_reason!r} reply_empty={reply is None}.")
        return {"reply": reply, "retryable": False, "billing_failure": False, "context_length_exceeded": False}
    except urllib.error.HTTPError as error:
        body_text = error.read().decode(errors="replace").strip()
        logger.error(f"DeepSeek API returned HTTP {error.code}: {body_text}")
        billing_failure = is_billing_failure(error.code, body_text)
        return {"reply": None, "retryable": error.code in RETRYABLE_STATUS_CODES and not billing_failure, "billing_failure": billing_failure, "context_length_exceeded": _is_context_length_error(error.code, body_text)}
    except (urllib.error.URLError, ConnectionError) as error:
        reason = getattr(error, "reason", error)
        logger.error(f"DeepSeek request failed at the connection level: {reason!r}")
        return {"reply": None, "retryable": not isinstance(reason, TimeoutError), "billing_failure": False, "context_length_exceeded": False}
    except Exception:
        ...
        return {"reply": None, "retryable": False, "billing_failure": False, "context_length_exceeded": False}
...
async def query_via_api(prompt: str, token: str, persona: str | None = None, session_dir: Path | None = None) -> str | dict | None:
    ...
    Returns:
        str | dict | None:
            The assistant's text reply; an "error" tool reply dict if the account's balance is exhausted; or None on any other failure/timeout.
    ...
    Notes:
        ...
        - A transient failure is retried by api_retry.py::run_with_retry(), which wraps each asyncio.wait_for() attempt - see this module's own header Notes. The return value is unchanged, still the reply text or None.
        - A context-length rejection is retried once with the oldest half of the remembered turns dropped - see this module's own header Notes. The trimmed history is persisted only if that retry succeeds, since nothing is written after a failure. A billing failure and a context-length rejection are mutually exclusive (a 402/429 versus a 400), so the one billing check below covers both requests.
        - Returns agent_errors.billing_exhausted_response() (a dict, not text, and never None) when DeepSeek reports the account's balance as exhausted (HTTP 402). Nothing is written to the transcript for it. That is the one case where this function returns a dict rather than str | None - see agent_errors.py.
    """
    agent = _resolve_agent(persona, session_dir)
    transcript = _read_transcript(session_dir)

    messages = _build_messages(agent["persona"], transcript["turns"], prompt)

    try:
        result = await run_with_retry(
            lambda: asyncio.wait_for(asyncio.to_thread(_post_chat_completion, messages, token, agent["model"], agent["reasoning_effort"]), timeout=_OUTER_TIMEOUT_SECONDS),
            "DeepSeek",
    ...
    if result["context_length_exceeded"]:
        turn_count_before = len(transcript["turns"]) // 2
        dropped_turns = _drop_oldest_half(transcript["turns"])
        if dropped_turns > 0:
            logger.warning(f"DeepSeek rejected the request as over the model's context length for session_dir={session_dir} - dropping the oldest {dropped_turns} of {turn_count_before} remembered turn(s) and retrying once.")
            messages = _build_messages(agent["persona"], transcript["turns"], prompt)
            try:
                result = await run_with_retry(
                    lambda: asyncio.wait_for(asyncio.to_thread(_post_chat_completion, messages, token, agent["model"], agent["reasoning_effort"]), timeout=_OUTER_TIMEOUT_SECONDS),
                ...
                logger.error(f"DeepSeek query (trimmed-history retry) for session_dir={session_dir} exceeded {_OUTER_TIMEOUT_SECONDS}s - abandoning it.")
                return None
        else:
            logger.error(f"DeepSeek rejected the request as over the model's context length for session_dir={session_dir}, but there is no older history to drop ({turn_count_before} remembered turn(s)) - the prompt or persona alone is too large.")
