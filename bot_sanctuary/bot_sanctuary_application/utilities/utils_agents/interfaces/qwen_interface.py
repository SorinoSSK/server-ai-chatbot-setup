...
#   - Session continuity via DashScope's own previous_response_id, remembered in a per-session_dir marker file
#     and subject to this application's own, stricter local TTL (settings.QWEN_SESSION_TTL_DAYS, optionally
#     aligned to settings.SESSION_RESET_TIME - see that setting's own Notes in config.py).
#
# Notes       :
...
def _post_response(prompt: str, token: str, persona: str | None, model: str, previous_response_id: str | None) -> dict:
    ...
    Returns:
        dict:
            {"reply": str | None, "response_id": str | None, "previous_response_id_rejected": bool} -
    ...
    try:
        ...
        return {"reply": _extract_output_text(payload), "response_id": payload.get("id") or None, "previous_response_id_rejected": False}
        ...
        return {"reply": None, "response_id": None, "previous_response_id_rejected": rejected}
    except Exception:
        ...
        return {"reply": None, "response_id": None, "previous_response_id_rejected": False}
...
def _session_expiry_cutoff(created_at: datetime) -> datetime:
    ...
    Returns:
        datetime:
            created_at plus settings.QWEN_SESSION_TTL_DAYS, optionally rolled forward to the next occurrence
            of settings.SESSION_RESET_TIME if one is configured.

    Notes:
        - settings.SESSION_RESET_TIME is unset (None) by default - the raw day-count cutoff is used as-is
          until a specific wall-clock time is configured. Deliberately the same, shared setting the global
          timed session reset schedule uses (utils_session/session_worker.py::trigger_timed_session_reset()),
          not a Qwen-specific one of its own - see config.py's own Notes on that setting for why.
        - When a specific time is configured, the cutoff is rolled forward (never backward) to the next
          occurrence on/after the raw day-count cutoff, same "never a boundary in the past" rule
          session_worker.py's own _seconds_until_next_session_reset_time() uses for the same setting.
    """
    cutoff = created_at + timedelta(days=settings.QWEN_SESSION_TTL_DAYS)
