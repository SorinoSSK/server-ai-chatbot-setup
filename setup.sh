...
get_masked_config_variables()
{
    ...
    echo "CHATBOT_SESSION_RESET_ALLOWED_CHAT_IDS"
    echo "CHATBOT_NAME"
    echo "CHATBOT_REDIS_USERNAME"
    echo "CHATBOT_REDIS_PASSWORD"
    echo "CHATBOT_LLM_CHAT_TYPE"
    echo "CHATBOT_LLM_ARCHITECT_TYPE"
    echo "CHATBOT_LLM_CODER_TYPE"
    echo "CHATBOT_LLM_REVIEW_TYPE"
    echo "CHATBOT_LLM_DOCUMENTATION_TYPE"
    echo "CHATBOT_LLM_CLAUDE_ACCESS_TYPE"
    echo "CHATBOT_LLM_CLAUDE_TOKEN"
    echo "CHATBOT_LLM_CODEX_ACCESS_TYPE"
    echo "CHATBOT_LLM_CODEX_TOKEN"
    echo "CHATBOT_LLM_DEEPSEEK_ACCESS_TYPE"
...
prompt_config_from_sample()
{
    ...
    while IFS= read -r variable_name; do
        if grep -q "^${variable_name}=\"REPLACE_WITH_${variable_name}\"" ./config.ini; then
            if [[ "${variable_name}" == "CHATBOT_TELEGRAM_ALLOWED_CHAT_IDS" || "${variable_name}" == "CHATBOT_SESSION_RESET_ALLOWED_CHAT_IDS" ]]; then
